...
#Compute instance for servers

resource "yandex_compute_instance_group" "servers" {
  name = "servers"
  service_account_id = yandex_iam_service_account.admin.id
  depends_on = [
    yandex_iam_service_account.admin,
...
resource "yandex_storage_bucket" "backup-backet-makar" {
  bucket = "backup-backet-makar"
  force_destroy = true
...
output "instance_group_servers_public_ips" {
  description = "Public IP addresses for server-nodes"
  value = yandex_compute_instance_group.servers.instances.*.network_interface.0.nat_ip_address
}

output "instance_group_servers_private_ips" {
  description = "Private IP addresses for server-nodes"
  value = yandex_compute_instance_group.servers.instances.*.network_interface.0.ip_address
}
...
#  value = yandex_lb_network_load_balancer.k8s-load-balancer.listener.*.external_address_spec.0.ip_address
}
